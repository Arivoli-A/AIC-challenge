...
def setup_ddp():
    if HAS_XLA and xr.device_type() == "TPU":
        device = xm.xla_device()
        local_rank = xm.get_local_ordinal()
        world_size = xm.xla_device_count()
        use_ddp = world_size > 1
        # No explicit init_process_group needed for TPU in basic usage
...
def cleanup_ddp():
    if not (HAS_XLA and xr.device_type() == "TPU") and torch.distributed.is_initialized():
