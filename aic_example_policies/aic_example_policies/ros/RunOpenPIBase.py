...
class RunOpenPIBase(Policy):
    ...
    def __init__(self, parent_node: Node):
        ...
        self.client = websocket_client_policy.WebsocketClientPolicy(host="34.168.160.195", port=8000)
        self.get_logger().info("OpenPI Base Policy initialized. Connected to VLA server.")
    ...
    def execute_task(
        ...
        **kwargs,
        ...
        self.get_logger().info(f"RunOpenPIBase.execute_task() started. Task: {task.task_id}")
        
        # Natural Language Prompt
        prompt = self.override_prompt if self.override_prompt else (task.task_description if task.task_description else "perform task")
